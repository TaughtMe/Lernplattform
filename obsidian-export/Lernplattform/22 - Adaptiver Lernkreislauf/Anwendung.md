# Adaptiver Lernkreislauf

## Zweck

Lernraum ist keine Sammlung einzelner Lernprogramme. Laufdiktat, LernBox, Deutsch, Kopfrechnen und Tastschreiben sind unterschiedliche Lernwege innerhalb eines gemeinsamen lokalen Förderkreislaufs.

> Übung erzeugt ein Lernsignal. Das Lernsignal aktiviert den fachlich passenden nächsten Schritt. Klassisch mitgeteilte Übungsaufträge werden vom Schüler im passenden freien Bereich geöffnet.

## Pädagogischer Kern

Lernraum fördert nachhaltig grundlegende Kompetenzen, insbesondere:

- Wortschatz und Vokabeln
- Rechtschreibung
- Memorisation und Merkspanne
- Kopfrechnen und grundlegende Aufgabenfamilien
- sicheres und genaues Tastschreiben

Es geht nicht primär um kurzfristige Vorbereitung auf einzelne Prüfungen. Schüler sollen regelmäßig und selbstständig an Grundlagen arbeiten können. Jede sinnvolle Beschäftigung wird anerkannt: fünf bearbeitete Wörter sind eine echte Lernleistung; hundert bearbeitete Wörter zeigen zusätzlichen Fleiß. Fehler erzeugen keine Minuspunkte, sondern einen besser passenden nächsten Lernschritt.

## Der gemeinsame Kreislauf

1. Die Lehrkraft erstellt eine Klasse und aktiviert die benötigten Module.
2. Schüler treten per Klassen-QR oder Code bei. Persönliche Lernstände bleiben lokal auf ihrem Gerät.
3. Die Lehrkraft kann einen Übungsbereich klassisch als Hausaufgabe oder Empfehlung mitteilen.
4. Schüler öffnen diesen Bereich selbst oder üben in den aktivierten Modulen frei.
5. Jede Bearbeitung erzeugt ein lokales, fachlich typisiertes Lernsignal.
6. **Heute üben** verbindet Fälligkeiten, frühere Fehler und adaptive Empfehlungen zu einer überschaubaren Auswahl.
7. Persönlicher Fortschritt würdigt Aktivität, Regelmäßigkeit, Verbesserung und Fleiß getrennt.
8. Nur ausdrücklich freigegebene, aggregierte Klassenbeiträge werden später per QR an das Lehrergerät zurückgegeben.

## Fachlich passende Reaktion

| Lernsignal | Nächster Lernweg |
|---|---|
| Vokabel falsch oder unsicher geschrieben | Karte in der LernBox kurzfristig fällig machen |
| Deutsches Wort wiederholt falsch | Lernwort und passende Rechtschreibstrategie aktivieren |
| Kleine Merkspanne oder viele Anläufe | kleinere Merkblöcke und gestufte Hilfen anbieten |
| Fehler beim Kopfrechnen | neue Aufgaben derselben Kompetenz- oder Aufgabenfamilie erzeugen |
| unsichere Tasten oder Buchstabenfolgen | passende Tippsequenzen und Wörter trainieren |

Ein Mathematikfehler wird niemals als Vokabelkarte gespeichert. Der gemeinsame Kern speichert das Lernsignal; das zuständige Fachmodul entscheidet über die passende Übung.

Der erste Mathematik-Kreislauf ist für die vier Grundrechenarten umgesetzt. Ein Fehler wird mit Rechenfamilie und Zahlenraum lokal gespeichert. Die anschließende Fehlerübung erzeugt neue Aufgaben derselben Rechenfamilie im gewählten Zahlenraum, statt lediglich die identische Rechnung erneut vorzulegen. Einmaleinsreihe, Nullregeln, negative Ergebnisse und Lückenform bleiben dabei Teil der gewählten Übungskonfiguration.

## Rolle von Laufdiktat und LernBox

Das Laufdiktat liefert kurzfristige, aussagekräftige Lernsignale: Fehlversuche, verwendete Hilfen, Merkspanne, Abfragerichtung und sicheren verdeckten Abruf. Die LernBox organisiert die langfristige Wiederholung von Vokabeln und getrennte Schreib- beziehungsweise Bedeutungsstände. Erst ihre Verbindung macht aus beiden Anwendungen den Lernraum.

## Wiederkehrende Oberfläche

Innerhalb einer Klasse verwendet Lernraum drei feste Einstiege:

1. **Heute üben:** fachlich passende nächste Schritte aus lokalen Lernsignalen
2. **Frei üben:** selbstbestimmtes Training grundlegender Kompetenzen; hier werden auch klassisch mitgeteilte Übungsaufträge geöffnet
3. **Mein Fortschritt:** Aktivität, Regelmäßigkeit und bewältigte frühere Schwierigkeiten

Das Laufdiktat steht außerhalb dieser Fachkacheln als gemeinsamer Raumcode-Einstieg. Deutsch öffnet direkt die Lernwörter, Vokabeln direkt die LernBox und Mathematik direkt das Kopfrechnen.

Module sind Werkzeuge innerhalb dieser vier Einstiege und nicht die oberste Produktstruktur. Eine aktive Lernrunde reduziert die Oberfläche auf Aufgabe, Fortschritt und unmittelbare Rückmeldung.

## Datenschutzgrenze

- Vollständige Antworten, Fehlerhistorie und persönlicher Förderbedarf bleiben lokal.
- Freies Üben wird nicht automatisch mit der Klasse geteilt.
- Rankingbeiträge bestehen nur aus begrenzten aggregierten Werten.
- QR-Übertragung und Raumcode dürfen keine permanente Online-Synchronisation der Lernhistorie einführen.

## Anerkennung und Motivation

Persönliches Lob und Klassenpunkte sind getrennt. Jeder Schüler erhält eine wertschätzende Rückmeldung für sinnvolle Aktivität. Klassenbeiträge würdigen Regelmäßigkeit, Verbesserung und begrenzten Fleiß, werden aber gedeckelt, damit absolute Leistung und unbegrenzte Menge nicht dominieren.

Siehe auch [[../01 - Plattform und Navigation/Anwendung|Plattform und Navigation]], [[../03 - Persönliche LernBox/Anwendung|Persönliche LernBox]], [[../06 - Lernlogik und Leitner-Boxen/Anwendung|Lernlogik und Leitner-Boxen]] und [[../10 - Häuser, Punkte und Motivation/Anwendung|Häuser, Punkte und Motivation]].
